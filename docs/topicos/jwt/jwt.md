...
gerar um JWT. Como já vimos, um JWT é composto por três partes codificadas:
cabeçalho, carga (_payload_) de declarações (*claims*) e assinatura. Para
gerar e assinar um token podemos utiliza a classe `io.smallrye.jwt.build.Jwt`,
veja um exemplo:
{: .fs-3 }
...
(*Issuer*) do JWT. Assim, no Quarkus/Microprofile devemos adicionar nos
serviços que recebem os tokens duas configurações no arquivo
`application.properties`:
{: .fs-3 }

* `mp.jwt.verify.issuer`: indica a url do emissor do token.
* `mp.jwt.verify.publickey.location`: indica a chave pública.
{: .fs-3 }
